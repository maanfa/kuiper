export interface CztOpenResult {
  valid: boolean
  error?: string
  tables: string[]
  tileCount: number
...
/** 打开并校验一个 .cztr 文件，返回结构化的校验结果 */
export async function openCztrFile(path: string): Promise<CztOpenResult> {
  try {
...
/** 查询指定 cztr 文件的某个表数据 */
export async function queryCztrTable(
  path: string,
  tableName: string,
  search?: string,
...
export async function queryCztrRow(
  path: string,
  tableName: string,
  whereCol: string,
  whereVal: unknown,
...
/** 查询瓦片信息 */
export async function queryCztrTileInfo(
  path: string,
  z: number,
  x: number,
  y: number,
...
/** 保存瓦片二进制数据 */
export async function saveCztrTileData(
  path: string,
  z: number,
  x: number,
  y: number,
